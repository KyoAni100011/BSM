...
//                UpdatePublisherController.handleTableItemSelection(id);
                FXMLLoaderHelper.loadFXML(new Stage(), "employee/bookPublisher/updatePublisher");
            } else {
                AlertUtils.showAlert("Error", "Can't find publisher", Alert.AlertType.ERROR);
            }
        } catch (IOException e) {
            AlertUtils.showAlert("Error", "Error loading update Publisher FXML", Alert.AlertType.ERROR);
        }
    }
    ...
    private void updatePublisherList() throws IOException {
